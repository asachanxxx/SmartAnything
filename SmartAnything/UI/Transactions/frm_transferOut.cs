...
{
    public partial class frm_transferOut : Form
    {
        int formMode = 0;
        string formID = "A0043";
        string formHeadertext = "Transfer Out Note";
        ...
        private static frm_transferOut objSingleObject = null;

        /// <summary>
        /// Singleton object creation method
        /// </summary>
        /// <returns>object of a frmM_Supplier</returns>
        public static frm_transferOut getSingleton()
        {
            if (objSingleObject == null || objSingleObject.IsDisposed)
            {
                objSingleObject = new frm_transferOut();
        ...
        public frm_transferOut()
        {
            InitializeComponent();
        }

        private void frm_transferOut_Load(object sender, EventArgs e)
        {
            try
            {
            ...
            try
            {
                switch (xenum)
                {
                    ...
                    case xEnums.PerformanceType.Save:
                            ...
                            if (UserDefineMessages.ShowMsg("", UserDefineMessages.Msg_PerfmBtn_Save, commonFunctions.Softwarename.Trim()) == System.Windows.Forms.DialogResult.Yes)
                            {
                                try
                                {
                                    using (System.Transactions.TransactionScope transaction = new System.Transactions.TransactionScope ())
                                    {
                                        ...
                                        objt_trnsferNote.sourceLocId = commonFunctions.GlobalLocation;
                                        objt_trnsferNote.date = dte_date.Value;
                                        objt_trnsferNote.refNo = txt_refNo.Text.Trim();
                                        objt_trnsferNote.remarks = txt_remarks.Text.Trim();
                                        objt_trnsferNote.destinationLocId = txt_destinationLocId.Text.Trim();
                                        objt_trnsferNote.purchaseReqNo = txt_purchaseReqNo.Text.Trim();
                                        objt_trnsferNote.noOfItems = commonFunctions.ToDecimal(txt_noOfItems.Text.Trim());
                                        objt_trnsferNote.noOfPeaces = commonFunctions.ToDecimal(txt_noOfPeaces.Text.Trim());
                                        objt_trnsferNote.grossAmount = commonFunctions.ToDecimal(txt_grossAmount.Text.Trim());
                                        objt_trnsferNote.isSaved = true;
                                        objt_trnsferNote.isProcessed = false;
                                        objt_trnsferNote.processDate = DateTime.Now;
                                        objt_trnsferNote.processUser = "";
                                        objt_trnsferNote.GLUpdate = false;
                                        objt_trnsferNote.triggerVal = 1;
                                        T_trnsferNoteDL bal = new T_trnsferNoteDL();
                                        bal.Savet_trnsferNoteSP(objt_trnsferNote, 1);
                                        ...
                                        foreach (DataGridViewRow drow in dataGridView1.Rows)
                                        {
                                            if (drow.Cells["Product Code"].Value.ToString().Trim() != null)
                                            {
                                                ...
                                                objt_transfer_detail.transferNoteNo = txt_no.Text.Trim();
                                                objt_transfer_detail.sourceLocId = commonFunctions.GlobalLocation;
                                                objt_transfer_detail.transferDate = dte_date.Value;
                                                objt_transfer_detail.stockCode = drow.Cells["Product Code"].Value.ToString();
                                                objt_transfer_detail.description = drow.Cells["Product Name"].Value.ToString();
                                                objt_transfer_detail.quantity = commonFunctions.ToDecimal(drow.Cells["Quntity"].Value.ToString());
                                                objt_transfer_detail.costPrice = commonFunctions.ToDecimal(drow.Cells["Cost Price"].Value.ToString());
                                                objt_transfer_detail.amount = commonFunctions.ToDecimal(drow.Cells["Amount"].Value.ToString());
                                                objt_transfer_detail.triggerVal = 1;
                                                T_transfer_detailDL baldet = new T_transfer_detailDL();
                                                baldet.Savet_transfer_detailSP(objt_transfer_detail, 1);
                                ...
                                if (UserDefineMessages.ShowMsg("", UserDefineMessages.Msg_PerfmBtn_Process, commonFunctions.Softwarename.Trim()) == System.Windows.Forms.DialogResult.Yes)
                                {
                                    ...
                                    T_trnsferNoteDL balprocess = new T_trnsferNoteDL();
                                    objt_trnsferNote = balprocess.Selectt_trnsferNote(objt_trnsferNote);

                                    objt_trnsferNote.isProcessed = true;
                                    objt_trnsferNote.processDate = DateTime.Now;
                                    objt_trnsferNote.processUser = commonFunctions.Loginuser;
                                    balprocess.Savet_trnsferNoteSP(objt_trnsferNote, 3);
                        ...
                        {
                            t_trnsferNote cat = new t_trnsferNote();
                            cat.no = txt_no.Text.Trim();
                            T_trnsferNoteDL dl = new T_trnsferNoteDL();
                            cat = dl.Selectt_trnsferNote(cat);
                            ...
                            {
                                errorProvider1.SetError(txt_no, "Transfer Number you have entered already processed.");
                                commonFunctions.SetMDIStatusMessage("Transfer Number you have entered already processed", 1);
                            }
                        }
                        break;
        ...
        private void UpdateStock()
        {
            try
            {
                t_transfer_detail trans = new t_transfer_detail();
                trans.transferNoteNo = txt_no.Text.Trim();
                List<t_transfer_detail> Transins = new List<t_transfer_detail>();
                Transins = new T_transfer_detailDL().SelectT_transfer_detailMulti(trans);

                foreach (t_transfer_detail det in Transins)
                {
                    //reduce stock from this location
                    T_StockDL.ReduceStock_Update(det.stockCode, commonFunctions.GlobalCompany, commonFunctions.GlobalLocation, det.quantity.Value);
                    ...
                    if (T_StockDL.ExistingT_Stock_new(det.stockCode.Trim(), commonFunctions.GlobalCompany, txt_destinationLocId.Text.Trim()))
                    {
                        ...
                        T_StockDL.AddStock_Update(det.stockCode, commonFunctions.GlobalCompany, txt_destinationLocId.Text.Trim(), det.quantity.Value);
                    }
                    else {
                        //insert
        ...
        private void txt_no_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (e.KeyCode == Keys.F2)
            {
                ...
                if (ActiveControl.Name.Trim() == txt_no.Name.Trim())
                {
                    int length = Convert.ToInt32(ConfigurationManager.AppSettings["TransferOutFieldLength"]);
                    string[] strSearchField = new string[length];

                    string strSQL = ConfigurationManager.AppSettings["TransferOutSQL"].ToString();

                    for (int i = 0; i < length; i++)
                    {
                        string m;
                        m = i.ToString();
                        strSearchField[i] = ConfigurationManager.AppSettings["TransferOutField" + m + ""].ToString();
                    }
        ...
        private void FindExisitingTransfer(string ReqNo)
        {
            try
            {
                if (T_trnsferNoteDL.ExistingT_trnsferNote(ReqNo.Trim()))
                {
                    formMode = 3;
                    //clear datagrid
                    dtx.Clear();
                    ...
                    t_transfer_detail req = new t_transfer_detail();
                    req.transferNoteNo = ReqNo.Trim();
                    T_transfer_detailDL tdl = new T_transfer_detailDL();
                    List<t_transfer_detail> requests = new List<t_transfer_detail>();
                    requests = tdl.SelectT_transfer_detailMulti(req);

                    foreach (t_transfer_detail det in requests)
                    {
