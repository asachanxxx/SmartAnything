    ...
    public partial class frm_chequeHandling : Form
    {
        ...
        private void LoadingData() {
            ...
            try
            {
                ...
                {
                    dt3 = T_RecDetDL.GetCQ("", rdo_daterange.Checked, rdo_post.Checked, rdo_retured.Checked, rdo_unrealized.Checked, rdo_realized.Checked, dte_to.Value, dte_from.Value);
                }
                else
                {
                ...
                foreach (DataGridViewRow drow in dte_cheques.Rows)
                {
                    if (drow.Cells[0].Value.ToString().Trim() != null)
                    {
                        ...
                        if (det != null)
                        {
                            ...
                            det.isReconsile = Convert.ToBoolean(drow.Cells[7].Value);
                            det.isreturned = Convert.ToBoolean(drow.Cells[8].Value);
                            det.ReasontoRtn = drow.Cells[8].Value.ToString(); 
            ...
            if (UserDefineMessages.ShowMsg("", UserDefineMessages.Msg_PerfmBtn_Save, commonFunctions.Softwarename.Trim()) == System.Windows.Forms.DialogResult.Yes)
            {
                foreach (DataGridViewRow drow in dte_cheques.Rows)
                {
                    if (drow.Cells[0].Value.ToString().Trim() != null)
                    {
                        ...
                        if (det != null)
                        {
                            det.isReconsile = Convert.ToBoolean(drow.Cells[8].Value);
                            det.isreturned = Convert.ToBoolean(drow.Cells[9].Value);
                            new T_RecDetDL().Savet_RecDetSP(det, 3);
                        }
